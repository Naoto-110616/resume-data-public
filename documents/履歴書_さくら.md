...
<div class="boxed boxed--motivation" style="min-height: 125mm;">
  <div class="box-label">志望の動機、特技、好きな学科、アピールポイントなど</div>
  <div class="box-content">■ 志望動機
日本のデジタルインフラを支える貴社で、その入口となる管理画面のUXに貢献したいと考え志望しました。現職で共通UIコンポーネントライブラリの設計・構築を主導した経験を、UI/UX設計から実装まで一貫して活かしたいです。

■ デザインから実装までの一貫対応力と共通UIコンポーネント構築
Figma のデザインを読み取りながら Next.js 実装、本番デプロイまでを独力で完結できることが強みです。直近では管理画面の共通 UI コンポーネントライブラリ（Radix UI ベース）を設計・構築し、社内導入・運用・展開まで主導、アクセシビリティと再利用性を意識したコンポーネント設計に取り組みました。デザインとエンジニアリングを横断できることが、UI / UX の品質向上に直結すると考えています。

■ プロセス改善と「仕組み」での品質保証
管理画面の UI コンポーネント共通化により、1ページあたりの実装時間を 1 日から半日へ短縮しました。あわせて、Shopify ↔ GitHub 自動デプロイによるデグレほぼゼロのリリースフロー整備や、Figma × GitHub Issue 連携による開発状況の見える化など、品質を「レビュー」ではなく「仕組み」で支える改善を推進してきました。
...
共通UIライブラリ（現職で構築・公開デモ）：https://next-shared-ui.vercel.app/</div>
</div>
