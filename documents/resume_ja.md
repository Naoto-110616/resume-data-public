...
■ プロセス改善とコンポーネント設計・品質保証
開発プロセスの改善とコンポーネント設計に継続的に取り組んできました。イオンリテール社のEC案件では、Figma と GitHub Issue を連携させて開発状況を可視化し、1ページあたりの実装時間を1日から半日へ短縮しました。また、ロジック層と UI 層を分けたディレクトリ構造により仕様変更時の影響範囲を局所化し、色 / アニメーション duration / opacity などを共通化、Storybook でコンポーネントを可視化することで、仕様変更に強く統一感のある実装を実現しています。Shopify ↔ GitHub 自動デプロイの提案・整備など、品質を「レビュー」ではなく「仕組み」で担保する取り組みも推進しています。
