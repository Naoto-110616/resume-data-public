...
<p class="body-text">
2021年に Web エンジニアとしてキャリアをスタートし、約5年間フロントエンド開発に従事。Next.js / TypeScript / Tailwind CSS を軸に EC サイト・コーポレートサイト・海外向け衣服レンタルサービスなど幅広い案件を担当してきました。直近ではリードポジションとして顧客折衝・要件定義・技術選定・コードレビューを一人称で担い、粗利 40% を達成するコスト設計や Shopify ↔ GitHub 自動デプロイの提案・定着化など、ビジネス成果に直結する提案を自ら推進してきました。デザインと技術の両軸でプロダクト品質を高めることへの関心も強く、Figma MCP・Claude・Cursor を活用したデザイン連携フローの構築や Storybook によるコンポーネント可視化など、デザイナーとエンジニアの協働を設計する役割も担ってきました。今後はフロントエンドリードとして、技術とビジネスの両面から事業に貢献できる環境を志向しています。
</p>

<h2 class="section-heading">得意とする分野・スキル</h2>

<ul class="skill-list">
  <li>ビジネス課題を起点とした技術提案・推進（見積もり・要件定義・コスト管理）</li>
  <li>UI/UX 設計からフロントエンド実装・本番デプロイまでの一貫対応</li>
  <li>共通 UI コンポーネントライブラリの設計・構築・社内展開・運用</li>
  <li>Web フロントエンド開発（Next.js / Tailwind CSS / Shopify Plus）</li>
  <li>デザイン再現性の高いコーディング（Figma MCP・Cursor 活用）</li>
...
備考：{{COMPANY_80}} の事業を引き継ぐ形で設立した新会社</div>
    ...
    <td>
      <div class="detail-block">
        ...
        <ul>
          ...
          <li>社内共通 UI コンポーネントライブラリ（Radix UI ベース）の設計・開発・社内展開を主導</li>
...
<table class="project-table">
  ...
  <tr>
    ...
    <td>
      ...
      <div class="detail-block">
        <div class="detail-label">▼実績</div>
        コンポーネント設計を担当し、ロジック層と UI 層を明確に分けたディレクトリ構造を採用、仕様変更に強い設計を実現しました。デザイン面では、色やアニメーションの duration、opacity などを共通化し、統一性を担保しました。Storybook を活用してコンポーネントの可視化・標準化を推進し、UI の再利用性と保守性を高めました。あわせて Figma と GitHub の Issue を連携し、開発状況の見える化にも取り組みました。スクラム開発のデイリーファシリテーションを担当し、心理的安全性の高い進行を支援しました。
      </div>
    </td>
  </tr>
...
<p class="body-text">
フロントエンドエンジニアとして5年のキャリアを積んだ現在、培った実装力・デザイン知見・ビジネス観点を統合し、より上流から事業に貢献できる環境へのステップアップを目指し、転職活動を開始しました。
</p>

<p class="body-text">
貴社を志望した理由は、「デザインと技術の融合」を会社の DNA として体現されている点に強く共感したためです。受託開発と自社プロダクトの二軸を持ちながら、人間中心設計の思想のもとで大手クライアントとともにプロダクトをつくる環境は、フロントエンドエンジニアとしてデザインと事業の両面に関わりたい自分のキャリア志向と合致しています。カジュアル面談を通じて「ビジネス観点を持って提案・推進できる人材」を求めていると伺いました。顧客折衝・要件定義・コスト設計・デプロイフロー改善など、実装だけに留まらずビジネス課題から逆算して動いてきた経験を、貴社でより大きなスケールで活かせると確信しております。フロントエンドリードとして技術的意思決定に関わりながら、デザインと事業の両面へキャリアを広げていくことを目指しています。
</p>

<h2 class="section-heading">自己PR</h2>

<p class="body-text">
<strong>ビジネス成果を起点に技術提案・推進できる点</strong>を強みとしています。顧客折衝から要件定義・技術選定・実装・リリースまでを一人称で担い、複数の実装方式（アプリ導入 / カスタム開発）の比較提案を通じて顧客予算と運用負荷の最適解を導いてきました。コスト管理では粗利 40% を達成する価格設計・スコープ管理を主導。Shopify ↔ GitHub 自動連携を提案・整備し、手動オペレーションに起因するデグレをほぼゼロに抑制しました。「仕様として落ちていない課題」を先に発見して提案する、コンサルティング的なスタンスで案件に向き合っています。
</p>

<p class="body-text">
<strong>デザイン再現性の高い実装と、仕組みで品質を担保する開発設計</strong>も得意としています。Next.js / TypeScript / Tailwind CSS を中心に、ロジック層と UI 層を分けたディレクトリ構造で仕様変更時の影響範囲を局所化し、テスタビリティとレビュー容易性を高めています。Storybook によるコンポーネント可視化や Shopify ↔ GitHub 自動デプロイの整備など、品質を「レビュー」ではなく「仕組み」で支える取り組みを推進しています。
</p>

<p class="body-text">
<strong>チーム横断のコミュニケーション</strong>も強みです。デイリースクラムのファシリテーション、オフショア・デザイン・ビジネスサイドとの連携、海外メンバーとの時差を考慮した非同期運用の整備など、リモート環境でもチームが機能する状態づくりに注力してきました。Linear チケットへの「受け入れ条件・参照リンク・既知の制約」の詳細記述や、レビュー指摘のナレッジ化など、属人化を防ぐ仕組みの整備を継続しています。
</p>

<p class="body-text">
<strong>デザインと技術の接点設計</strong>への関心が強く、デザイナーとエンジニアが協働しやすい環境づくりを意識して取り組んできました。Figma MCP・Claude・Cursor を活用したデザイン読み取りから実装までの連携フロー構築、Adobe XD から Figma への移行提案、コンポーネント命名のデザイナーとのすり合わせ、色 / アニメーション duration / opacity の共通化など、デザイントークンに通じる運用設計を実践してきました。「事故らない UI」を実装レベルで実現する意識と、デザイン観点からのフィードバックをエンジニア視点で返す姿勢を大切にしています。
</p>
